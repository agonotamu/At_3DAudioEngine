...
*/
    void At_WfsSpatializer::applyWfsGainDelay(int virtualMicIdx, int bufferLength, bool isDirective) { //modif mathias 06-17-2021
        ...
        {
            //modif mathias 07-26-2021
            if (isDirective == true) {
#if DIRECTIVE_PLAYER
                float channelOne, channelTwo;
                m_pCircularDelayMultiChannelBuffer[(int)m_ChannelWeight[virtualMicIdx][0][0]].Read(channelOne);
                m_pCircularDelayMultiChannelBuffer[(int)m_ChannelWeight[virtualMicIdx][1][0]].Read(channelTwo);
                m_pTmpMonoBuffer_in[sample] = m_pWfsVolume[virtualMicIdx] *
                    (m_ChannelWeight[virtualMicIdx][0][1] * channelOne
                        + m_ChannelWeight[virtualMicIdx][1][1] * channelTwo);
#endif
            }
                ...
                m_pTmpMonoBuffer_in[sample] = /*m_pWfsVolume[virtualMicIdx] **/ circularbuffer;
#ifdef RING_BUFFER
                std::cout << "ReadSample : " << m_pTmpMonoBuffer_in[sample] << "\n";
#endif
...
*/
    ...
    void At_WfsSpatializer::updateMultichannelDelayBuffer(float* inBuffer, int bufferLength, int inChannelCount) {
        ...
        */
        void At_WfsSpatializer::updateMultichannelDelayBuffer(float* inBuffer, int bufferLength, int inChannelCount) {
            for (int sample = 0; sample < bufferLength; sample++) {
                for (int channel = 0; channel < inChannelCount; channel++) {
                    m_pCircularDelayMultiChannelBuffer[channel].Feed(inBuffer[sample]);
                }
            }
        }
#endif
...
#ifdef RING_BUFFER
            std::cout << "FeedSample : " << m_pTmpMonoBuffer_in[sample] << "\n";
#endif
        }
    }
...
#ifdef RING_BUFFER
                std::cout << "OutSample " << outBuffer[m_virtualMicCount * sampleIndex + virtualMicIndex] << "\n";
#endif
