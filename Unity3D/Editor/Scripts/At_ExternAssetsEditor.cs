...
public class At_ExternAssetsEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        using (new GUILayout.HorizontalScope())
        {
            if (externAssetsState.externAssetsPath_audio != null && externAssetsState.externAssetsPath_audio.Length > 0)
            {
        ...
        {
            // Display and test if the Button "Open" has been clicked
            if (GUILayout.Button("States Extern Asset Folder (Editor)"))
            {
                externAssetsPaths = StandaloneFileBrowser.OpenFolderPanel("Select Folder", "", false);
        ...
        using (new GUILayout.HorizontalScope())
        {
            if (externAssetsState.externAssetsPath_state != null && externAssetsState.externAssetsPath_state.Length > 0)
            {
